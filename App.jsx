...
export default function RosalyssWandersSite() {
  ...
  // Section 24 — capture ?ref=CODE from the URL automatically
  useEffect(() => {
    try {
  ...
  async function handleSubmit(ev) {
    ...
    setSubmitting(true);
    try {
      // 1. resolve referral code -> referrer_id (if any)
      let referrerId = null;
      if (form.referral.trim()) {
        try {
          const rows = await sb(`rpc/lookup_referral_code`, {
            method: "POST",
            body: JSON.stringify({ p_code: form.referral.trim().toUpperCase() }),
          });
          if (rows && rows[0]) referrerId = rows[0].referrer_id;
        } catch (_) { /* invalid code is not fatal to the inquiry */ }
      }

      // 2. create the customer record via RPC (security definer function) —
      //    avoids needing a public SELECT policy on the customers table,
      //    which would otherwise expose all customers' PII to anyone.
      const customerId = await sb("rpc/create_public_customer", {
        method: "POST",
        body: JSON.stringify({
      ...
      // 3. create the inquiry, linked to the customer (and referrer, if any)
      await sb("inquiries", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          airport_origin: form.airport || null,
          travel_start_date: form.startDate || null,
          travel_end_date: form.endDate || null,
          flexible_promo_dates: form.flexible === "Yes",
          adults: Number(form.adults),
          children: Number(form.children),
          child_ages: form.childAges || null,
          airfare_required: form.airfare === "Yes",
          preferred_hotel: form.hotel || null,
    ...
    <div className="min-h-screen bg-[#FBF7EF] text-[#0F2A43] antialiased" style={{ fontFamily: "'Inter', ui-sans-serif, system-ui" }}>
      ...
      <footer className="bg-[#0F2A43] py-12 text-white/70">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="mt-10 border-t border-white/10 pt-6 text-[11.5px] text-white/50">
            © 2026 Rosalyss Wanders Travel and Tours Services. All Rights Reserved.
          </div>
        </div>
      </footer>
    </div>
...
/* ---------------------------------------------------------
   LOGO MARK — sunset / plane / island motif echoing the brand seal
--------------------------------------------------------- */
function LogoMark({ dark }) {
  return (
    <svg width="38" height="38" viewBox="0 0 38 38" fill="none">
      <circle cx="19" cy="19" r="18" fill={dark ? "#FBF7EF" : "#0F2A43"} />
      <circle cx="19" cy="15" r="6" fill="#E8A33D" />
      <path d="M4 26c3-3 7-5 15-5s12 2 15 5" stroke="#1C7C8C" strokeWidth="2.4" strokeLinecap="round" fill="none" />
      <path d="M9 20l9-5 5 1.6-6 4" fill={dark ? "#0F2A43" : "#FBF7EF"} />
    </svg>
  );
}
